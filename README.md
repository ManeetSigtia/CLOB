...
| **Why avoid a positional `pop()` for matching?**<br/>"Why use specific order removal instead of just popping the top of the heap?" | Popping the best price off the heap creates **Temporal Coupling**—it assumes the heap top is always valid (i.e. the price on the heap always has a corresponding doubly linked list in the dictionary) and requires a cleanup function to be called immediately prior to avoid a `NoneType` crash. The success of `pop()` becomes dependent on the timing of when `_delete_best_cancelled_orders()` was last called. Furthermore, `pop()` is "blind"; it guesses what to remove based on the heap's state. By using **Surgical Order Removal** (`delete_order`), the matching engine removes the exact `Order` object it just filled. This is identity-based rather than position-based, making the engine robust against "ghost" prices (stale heap entries) and eliminating redundant $O(\log P)$ heap lookups during a match sequence.  |
